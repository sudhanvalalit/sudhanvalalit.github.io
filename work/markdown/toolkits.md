# Toolkits

![Software Tools](../img/Computation/comp-bg.jpeg)

Software toolkits provide accessible interfaces to complex scientific data and computational methods. My work contributes to developing open-source tools that democratize access to nuclear physics data and enable reproducible research across the community.

## Key Publications

### The nucleardatapy toolkit for simple access to experimental nuclear data, astrophysical observations, and theoretical predictions

Margueron, Jérôme, Drischler, Christian, Dutra, Mariana, Gandolfi, Stefano, Gezerlis, Alexandros, et al.

- [arXiv:2506.20434](https://arxiv.org/abs/2506.20434)
- [GitHub Repository (nucleardatapy)](https://github.com/jeromemargueron/nucleardatapy)
- **Citations:** 2
